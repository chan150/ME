...
object DMatrixSerializer {
  def serialize(obj: DistributedMatrix): InternalRow ={
  ...
  def deserialize(datum: Any): DistributedMatrix ={
    datum match {
      case row: InternalRow =>
        require(row.numFields == 7, s"MatrixUDT.deserialize give row with length ${row.numFields} but requires length == 7")
